...
GLuint load_shader(GLenum type, const std::string& filepath)
{
    ...
    long size = ftell(file); // kysyt‰‰n filukoko
    fseek(file, 0, SEEK_SET); // kelataan alkuun
    // varataan muistia tekstille +1 merkki nul terminaattorille
    char* buffer = (char*)malloc(size + 1); 
    fread(buffer, 1, size, file); // luetaan koko filu bufferiin
    buffer[size] = '\0';
    fclose(file);
...
int main(int argc, char* argv[])
{
    ...
    static const float vertices[] = {
        ...
        1.0f, -1.f, 0.0f, 0.1f, 0.2f, 0.8f, 1.0f,
        0.0f, 1.0f, 0.0f, 0.8f, 0.9f, 0.2f, 1.0f
    };
